...
def test_test_endpoint_falls_back_to_server_settings(client, mock_llm_complete):
    res = client.post("/api/llm/test")
    assert res.status_code == 200
    assert res.json()["model"] == "gpt-4o-mini"  # from fake_settings
...
def test_complete_uses_byok_key(client, mock_llm_complete):
    ...
    }, headers={"x-api-key": "user-key", "x-model": "gpt-4o"})
    assert res.status_code == 200

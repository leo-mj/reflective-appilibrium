...
const _inlineState = {
  ...
  elements: [
    ...
    // { id: "T1", type: "theory", status: "active", confidence: "high", origin: "llm", text: "...", addedRound: 5 },
...
// ============================================================
const SAMPLE_STATE = import.meta.env.VITE_USE_DUMMY_STATE
  ? _dummyState
  : _inlineState;
      ...
      {dims.w > 0 && (
        ...
        >
          ...
          {visRels.map((r, i) => {
            ...
            const isW =
              sEl?.status === "withdrawn" || tEl?.status === "withdrawn";
            const sr =
              sEl?.type === "principle" ? 28 : sEl?.type === "theory" ? 22 : 18;
            const tr =
              tEl?.type === "principle" ? 28 : tEl?.type === "theory" ? 22 : 18;
            const dx = tp.x - sp.x,
              dy = tp.y - sp.y;
          ...
          {visibleEls.map((e) => {
            ...
            const r =
              e.type === "principle" ? 28 : e.type === "theory" ? 22 : 18;
            return (
              <g
                key={e.id}
                transform={`translate(${pos.x},${pos.y})`}
                style={{ cursor: "pointer", transition: TRANSITION }}
                onMouseEnter={(ev) => {
              ...
              >
                ...
                <text
                  ...
                  dy="0.35em"
                  fill={isW ? "#666" : "#fff"}
                  fontSize={e.type === "principle" ? 13 : 11}
                  fontWeight="bold"
                  style={{
      ...
      {tooltip && (
        ...
        >
          ...
          {tooltip.el.previousText && (
            ...
            >
              Previously: {tooltip.el.previousText}
            </div>
          )}
          {tooltip.el.reason && (
            <div
            ...
            >
              Withdrawn: {tooltip.el.reason}
            </div>
          )}
          <div style={{ color: C.dim, fontSize: 10, marginTop: 4 }}>
            Confidence: {tooltip.el.confidence} · Origin: {tooltip.el.origin}
          </div>
        </div>
      )}
    </div>
      ...
      >
        ...
        {tooltip && (
          ...
          >
            ...
            <div style={{ color: C.dim, fontSize: 10, marginTop: 4 }}>
              Confidence: {tooltip.el.confidence} · Origin: {tooltip.el.origin}{" "}
              · Added: Round {tooltip.el.addedRound || "?"}
            </div>
          </div>
        )}
      </div>
...
function TextTab({ state, showWithdrawn }) {
  ...
  const renderEl = (e) => {
    ...
    if (e.status === "revised" && e.previousText)
      st = `revised, from: "${e.previousText}"`;
  ...
  state.relations.forEach((r) => {
    ...
    if (
      ...
      pCovers[f.id]?.push(to.id);
    if (
      to?.type === "principle" &&
      f?.type === "judgment" &&
      r.type === "supports"
    )
      pCovers[to.id]?.push(f.id);
  });
  const visRels = state.relations.filter(
    (r) => visIds.has(r.from) && visIds.has(r.to),
  );
  ...
  tx +=
    ...
    p
      .map(
        (pr) =>
          renderEl(pr) +
          (pCovers[pr.id]?.length
            ? `\n    covers: ${pCovers[pr.id].join(", ")}`
            : ""),
      )
      .join("\n") +
    "\n\n";
  if (t.length)
    tx += "BACKGROUND THEORIES\n" + t.map(renderEl).join("\n") + "\n\n";
  tx +=
    "RELATIONS\n" +
    visRels
      .map((r) => `  ${r.from} → ${r.type} → ${r.to}: ${r.explanation}`)
      .join("\n") +
    "\n\n";
  tx += "COHERENCE\n";
  ...
  tx +=
    ...
    state.log
      .map(
        (l) =>
          `  Round ${l.round}: ${l.findings} ${l.decision !== "—" ? `Decision: ${l.decision}` : ""}`,
      )
      .join("\n");
  return (
    <pre
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minHeight: 0, marginTop: 4 }}>
        ...
        {tab === "text" && (
          <TextTab state={state} showWithdrawn={showWithdrawn} />
        )}
        {tab === "history" && (
          <HistoryTab state={state} positions={positions} />
        )}
      </div>
    </div>
  );

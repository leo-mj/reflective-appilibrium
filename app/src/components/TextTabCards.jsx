...
import {
  GHOST_BTN_STYLE, WITHDRAW_BTN_STYLE, CARD_STYLE, META_LABEL_STYLE, CONTENT_FONT_SIZE,
...
export function Highlight({ text, query }) {
  ...
  return parts.map((part, i) =>
    i % 2 === 1 ? (
      <mark key={i} style={{ background: C.supports + "44", color: "inherit", borderRadius: 2, padding: "0 1px" }}>
        {part}
      </mark>
    ) : part,
  );
  ...
  return (
    ...
    >
      <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
        {onToggle && (
          <span style={{ fontSize: 10, transition: "transform 0.15s", display: "inline-block", transform: collapsed ? "rotate(-90deg)" : "rotate(0deg)" }}>▼</span>
        )}
        {title}
      </span>
      {onAdd && (
        <button
          ...
          style={{ ...GHOST_BTN_STYLE, fontSize: 13, padding: "0 5px 1px", fontWeight: "bold", letterSpacing: 0, textTransform: "none" }}
        >+</button>
      )}
    </div>
  );
...
export function ElementCard({ e, dim }) {
  const { pCovers, onEditRequest, onWithdrawRequest, badgeColor, search } = useContext(Ctx);
  const isW = e.status === "withdrawn";
  const color = badgeColor(e.id);
  return (
    <div style={{ ...CARD_STYLE, opacity: dim ? 0.4 : isW ? 0.55 : 1, borderLeft: `3px solid ${color}`, paddingLeft: 10 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6, marginBottom: 5 }}>
        ...
        <ActionButtons onRevise={() => onEditRequest(e.id)} onWithdraw={!isW ? () => onWithdrawRequest(e.id) : null} />
      </div>
      <div style={{ fontSize: CONTENT_FONT_SIZE, color: isW ? C.dim : C.text, lineHeight: 1.65, textDecoration: isW ? "line-through" : "none" }}>
        <Highlight text={e.text} query={search} />
      </div>
      {e.previousText && <div style={{ ...META_LABEL_STYLE, color: C.dim }}>Previously: "{e.previousText}"</div>}
      {e.reason        && <div style={{ ...META_LABEL_STYLE, color: C.dim }}>Withdrawn: {e.reason}</div>}
    </div>
  );
}
...
export function RelationCard({ r, dim }) {
  const { state, selectedRel, onSelectRel, onSelect, onEditRelRequest, onWithdrawRelRequest, badgeColor, search } = useContext(Ctx);
    ...
    <div style={{ ...CARD_STYLE, opacity: dim ? 0.4 : 1 }}>
      <div
        ...
        style={{
          ...
          cursor: "pointer", borderRadius: 4, padding: "2px 4px", margin: "0 -4px 8px",
      ...
      <div style={{ fontSize: CONTENT_FONT_SIZE, color: C.dim, lineHeight: 1.5, fontStyle: "italic" }}>
...
export function CoherenceGroup({ title, color, items }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 10, color, fontWeight: "bold", letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>{title}</div>
      {items.map((item, i) => (
        <div key={i} style={{ fontSize: 12, color: C.dim, marginBottom: 6, lineHeight: 1.5, paddingLeft: 8, borderLeft: `2px solid ${color}55` }}>
          {item}
        </div>
      ))}
    </div>
...
export function HighlightedSection({ selectedRel, selected, selectedEl, neighbourEls, hlRels, restEls, restRels }) {
  return (
    <>
      {selectedRel ? (
        <>
...
export function SectionListing({
  refJudgments, refPrinciples, refTheories, refRelations,
  displayEls, displayRels, isCollapsed, toggle, openAddElement, openAddRelation,
}) {
  const byType = (type) => displayEls.filter((e) => e.type === type);
  return (
    <>
      <div ref={refJudgments}>
        <SectionHeader title={`Judgments (${byType("judgment").length})`} onAdd={openAddElement} collapsed={isCollapsed("judgments")} onToggle={() => toggle("judgments")} />
        {!isCollapsed("judgments") && byType("judgment").map((e) => <ElementCard key={e.id} e={e} />)}
      </div>
      <div ref={refPrinciples}>
        <SectionHeader title={`Principles (${byType("principle").length})`} onAdd={openAddElement} collapsed={isCollapsed("principles")} onToggle={() => toggle("principles")} />
        {!isCollapsed("principles") && byType("principle").map((e) => <ElementCard key={e.id} e={e} />)}
      </div>
      <div ref={refTheories}>
        <SectionHeader title={`Background Theories (${byType("theory").length})`} onAdd={openAddElement} collapsed={isCollapsed("theories")} onToggle={() => toggle("theories")} />
        {!isCollapsed("theories") && byType("theory").map((e) => <ElementCard key={e.id} e={e} />)}
      </div>
      <div ref={refRelations}>
        <SectionHeader title={`Relations (${displayRels.length})`} onAdd={openAddRelation} collapsed={isCollapsed("relations")} onToggle={() => toggle("relations")} />
        {!isCollapsed("relations") && displayRels.map((r, i) => <RelationCard key={i} r={r} />)}
      </div>
    </>
  );
}

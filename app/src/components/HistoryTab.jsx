 ...
 */
function RoundScoresChart({ roundScores, snappedRound }) {
  ...
  // Only render rounds that have scores
  ...
  // Vertical marker for the current playback round
    ...
    <div ref={containerRef} style={{ position: "relative", marginBottom: 4 }}>
      ...
      >
        <g transform={`translate(${m.left},${m.top})`}>
          ...
          {/* Vertical marker — current playback round */}
          {markerX != null && (
            <line
          ...
          {/* Dots — future rounds at reduced opacity */}
...
export function HistoryTab({ state, positions, onRoundChange, isWide }) {
  ...
  const computeScores = async () => {
    ...
    setScoreError(null);
    try {
      const data = await scorePerRound(state);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      {/* Z-scores across rounds */}
      <div style={{ paddingBottom: 6 }}>
        {roundScores ? (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: scoresOpen ? 6 : 0 }}>
              <button
              ...
              <button
                ...
                style={{
                  ...
                  opacity: loadingScores ? 0.5 : 1,
                }}
              >
                {loadingScores ? "Computing…" : "Recompute"}
              </button>
            </div>
            {scoresOpen && (
              <RoundScoresChart roundScores={roundScores} snappedRound={snappedRound} />
            )}
          </>
        ) : (
          <button
          ...
          >
            {loadingScores ? "Computing Z-scores…" : "Compute Z-scores"}
          </button>
        )}
        {scoreError && (
          <div style={{ fontSize: 11, color: C.conflicts, marginTop: 4 }}>{scoreError}</div>
        )}
      </div>

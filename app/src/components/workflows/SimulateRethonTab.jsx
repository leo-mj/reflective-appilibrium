...
function deriveEquilibrium(result) {
  ...
  return { retained, withdrawn, finished: result.translated_re_state.finished, steps: evolution.length };
}
...
function ElementRow({ element, faded = false }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: faded ? C.dim : color, fontWeight: "bold", fontSize: 11, marginRight: 6 }}>
        {element.id}
      </span>
...
function EvolutionStep({ step, position }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 6, fontSize: 11 }}>
      <span style={{ color: C.dim, minWidth: 24, textAlign: "right", paddingTop: 2 }}>{step}</span>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
        {position.length === 0
          ? <span style={{ color: C.dim }}>∅</span>
          : position.map((e, i) => <IdBadge key={i} element={e} negated={e.negated} />)
        }
      </div>
    </div>
  );
 ...
 */
export function SimulateRethonTab({ state, useDummy = false, onApplyRethonEquilibrium }) {
  ...
  const equilibrium = useMemo(() => result ? deriveEquilibrium(result) : null, [result]);
      ...
      <div style={{ overflowY: "auto", flex: 1, padding: "0 4px 24px" }}>
        ...
        {activeCount < 3 && (
          <div style={{ fontSize: 12, color: C.dim }}>
            Add at least three active elements to run the simulation.
          </div>
        )}
        ...
        {equilibrium && (
          <>
            ...
            {result.translated_arguments.length === 0
              ...
              : result.translated_arguments.map((arg, i) => <ArgumentCard key={i} argument={arg} />)
            }
            ...
            <button
              ...
              style={{
                ...
                color: C.dim, borderRadius: 6, padding: "4px 10px",
                fontSize: 11, cursor: "pointer", marginBottom: evolutionOpen ? 10 : 0,
              }}
            >
              {evolutionOpen ? "Hide" : "Show"} steps
            </button>
            {evolutionOpen && result.translated_re_state.evolution.map((pos, i) => (

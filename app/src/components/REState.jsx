...
function stateAtRound(state, round) {
  ...
  return {
    ...
    relations: state.relations.filter(
      r => visIds.has(r.from) && visIds.has(r.to) && (r.addedRound || 1) <= round
    ),
  };
}
...
function useREActions(initialState) {
  ...
  const handleSelectNode = (updater) => { setSelectedRel(null); setSelected(updater); };
  const handleSelectRel  = (updater) => { setSelected(null);    setSelectedRel(updater); };
  ...
  const handleEditSave = (formData) => {
    ...
    const newEl = { ...oldEl, ...formData, status: "revised",
                    previousText: oldEl.text, revisedRound: newRound };
    ...
    setState(prev => ({
      ...
      log: [...prev.log, makeLogEntry(newRound, `${oldEl.id} was edited by the user.`,
        "Changes applied", diffs.length ? diffs.join("; ") : "No fields changed")],
    }));
    setEditingEl(null);
  };
  ...
  const handleRelEditSave = (formData) => {
    ...
    setState(prev => ({
      ...prev, round: newRound,
      relations: prev.relations.map(r => r === editingRel
        ? { ...editingRel, ...formData, status: "revised", revisedRound: newRound } : r),
      log: [...prev.log, makeLogEntry(newRound,
  ...
  const handleWithdrawRequest = (elementId) => {
    ...
    setState(prev => ({
      ...
      elements: prev.elements.map(e => e.id === elementId
        ...
        : e),
      log: [...prev.log, makeLogEntry(newRound, `${elementId} was withdrawn by the user.`,
        "Withdrawn", `${elementId}: status → withdrawn`)],
    }));
  };
  ...
  const handleWithdrawRelRequest = (rel) => {
    ...
    setState(prev => ({
      ...
      relations: prev.relations.map(r => r === rel
        ? { ...r, status: "withdrawn", withdrawnRound: newRound } : r),
      log: [...prev.log, makeLogEntry(newRound,
        `Relation ${rel.from} → ${rel.to} was withdrawn by the user.`,
        "Withdrawn", `${rel.from} → ${rel.to}: status → withdrawn`)],
    }));
  };
  ...
  const handleAddRelation = (formData) => {
    ...
    setState(prev => ({
      ...
      log: [...prev.log, makeLogEntry(newRound,
        ...
        "Added", `${formData.from} → ${formData.to} (${formData.type}) added`)],
    }));
  ...
  return {
    state,
    selected, selectedRel, handleSelectNode, handleSelectRel,
    editingEl,  setEditingEl,  handleEditRequest, handleEditSave,
    editingRel, setEditingRel, handleRelEditSave,
    handleWithdrawRequest, handleWithdrawRelRequest,
    handleAddElement, handleAddRelation,
  };
}
...
function AppHeader({ round, topic, tab, setTab, showText, setShowText, onHome }) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
      <div>
        <div style={{ fontSize: 16, fontWeight: "bold" }}>Reflective Equilibrium — Round {round}</div>
      ...
      <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
        ...
        <button onClick={onHome} style={{ ...btn(false), marginLeft: 50 }}>← Home</button>
      </div>
    </div>
  );
}
...
function GraphPanel({ tab, state, positions, showWithdrawn, setShowWithdrawn,
                      selected, onSelect, selectedRel, onSelectRel,
                      onAddElement, onAddRelation, onRoundChange }) {
  return (
    <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <Legend />
      <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: C.dim, cursor: "pointer" }}>
        <div onClick={() => setShowWithdrawn(s => !s)} style={{
      ...
      <div style={{ flex: 1, minHeight: 0, marginTop: 4 }}>
        {tab === "graph" && (
          <Graph state={state} showWithdrawn={showWithdrawn} positions={positions}
            ...
            selectedRel={selectedRel} onSelectRel={onSelectRel}
...
function EditModals({ editingEl, setEditingEl, onEditSave, editingRel, setEditingRel, onRelEditSave, round }) {
  return (
    <>
      {editingEl && (
        <EditModal element={editingEl} currentRound={round}
          onSave={onEditSave} onCancel={() => setEditingEl(null)} />
      )}
      {editingRel && (
        <EditRelationModal relation={editingRel} currentRound={round}
          onSave={onRelEditSave} onCancel={() => setEditingRel(null)} />
      )}
    </>
  );
}
...
export default function REState({ initialState, onHome, onReady }) {
  ...
  const { state, selected, selectedRel, handleSelectNode, handleSelectRel,
          editingEl, setEditingEl, handleEditSave,
          editingRel, setEditingRel, handleRelEditSave,
  ...
  const { positions, ready } = useStablePositions(state, { w: graphW, h: dims.h });
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: isWide ? "row" : "column", gap: 12 }}>
        ...
        <GraphPanel tab={tab} state={state} positions={positions}
          showWithdrawn={showWithdrawn} setShowWithdrawn={setShowWithdrawn}
          selected={selected} onSelect={handleSelectNode}
          selectedRel={selectedRel} onSelectRel={handleSelectRel}
          onAddElement={handleAddElement} onAddRelation={handleAddRelation}
      ...
      <EditModals
        editingEl={editingEl} setEditingEl={setEditingEl} onEditSave={handleEditSave}
        editingRel={editingRel} setEditingRel={setEditingRel} onRelEditSave={handleRelEditSave}
        round={state.round} />
    </div>
  );
}
